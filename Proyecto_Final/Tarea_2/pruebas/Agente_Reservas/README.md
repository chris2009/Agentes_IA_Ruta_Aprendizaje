...
mejor se ajustan al `agent_card_profile_reserva_context_memory_reflex_v3.html`
(Agent Profile Card — Fase 2 · v3 de "Clemente Reservations"). Sigue la
misma convención que `/Examples`: cada script es autocontenido (LangChain
1.x `create_agent` + Ollama `llama3.2`), con su propio mini-entorno
simulado de restaurante, y puede ejecutarse de forma independiente.

## Cumplimiento de la rúbrica de evaluación

| Criterio | Cómo se cumple |
|---|---|
| System Prompt asociado a cada tipo de agente | `02`, `03` y `04` usan `system_prompt=`/`dynamic_prompt` explícito. `01` y `04` usan `dynamic_prompt` (equivalente funcional: genera el system prompt en cada paso inyectando el modelo interno), el mismo mecanismo que usa `02_model_based_reflex_agent.py` en `/Examples` para este tipo de agente. |
...
| Short-term memory conforme a ejemplos de sesiones anteriores | Cada archivo incluye un bloque `EJEMPLOS_SESIONES_ANTERIORES` (few-shot) que enseña el patrón esperado, y los 4 agentes arrastran el historial de mensajes entre turnos (`continuar_conversacion`), demostrado con una conversación multi-turno en `__main__` donde el agente no vuelve a pedir datos ya dados antes en la misma sesión. |
| Actualización del Profile Card en base al uso de memoria | **Hecho**: `agent_card_profile_reserva_context_memory_reflex_v3.html` documenta el presupuesto de Context Window, la arquitectura de memoria completa (CoALA) y el inventario de los 4 scripts. |
...
complementarios** (didácticos, uno por capa) más **un cuarto agente que los
unifica** para uso como producto, cada uno mapeado a secciones concretas
del profile card, más un motivo explícito para **descartar** los otros 2
tipos de IBM:
...
| 4 | `04_unified_agent.py` | **Model-Based + Goal-Based + Utility-Based, unificados** | Todas las anteriores en un solo agente de producto: memoria + meta + recomendación rankeada con fallback para cliente nuevo. Adaptado del aporte de equipo de Jesús Barboza. |
| ✗ | — | Simple Reflex | **Descartado como arquitectura base**: el profile card exige memoria explícita; un reflejo simple no tiene estado. El patrón condición-acción sigue vivo como guardrail dentro de `02/03/04` (ej. "sin confirmación explícita → no registrar"). |
| ✗ | — | Learning | **Descartado por ahora**: no hay, en el profile card actual, un crítico/loop de feedback definido; se propone como evolución v2 |
...
### 2.4 `04_unified_agent.py` — agente de producto (unifica 1+2+3)

Los archivos `01/02/03` son **didácticos**: cada uno aísla una sola capa
(memoria / meta / utilidad) y le quita el resto para que ese patrón de IBM
se lea limpio. Por diseño, **no están apilados entre sí**. El `04` los
colapsa en un solo agente coherente, que es la arquitectura natural para
producción:

- Carga `perfil_cliente` (long-term memory) igual que `01`.
- Persigue la meta de una reserva confirmada con replanificación y
  escalamiento igual que `02`.
- Cuando la hora exacta no está libre, recomienda **varias alternativas
  rankeadas por utilidad** (igual que `03`), con una diferencia clave: el
  score se **adapta solo** según si el cliente es conocido (pondera su
  `zona_favorita`) o nuevo (zona en valor neutro, ranking por hora +
  capacidad).
- Agrega una **regla reactiva** de orientación temprana
  (`resumen_disponibilidad_dia`): si el cliente da una fecha pero aún no
  dice cuántas personas son, el agente muestra las franjas abiertas de ese
  día mientras pide el dato que falta, en vez de quedarse callado. Tener
  esta regla no baja la clasificación del agente: un utility-based
  normalmente incluye reglas reactivas como atajos/guardrails.
- Sigue exigiendo confirmación explícita antes de `confirmar_reserva` y
  escala de inmediato ante alergias o casos especiales.

Este archivo fue adaptado del aporte de equipo de **Jesús Barboza**
(`github.com/JesusBarboza1994/ai_agents_utec`), quien construyó el agente
unificado sobre estos mismos `01/02/03`. Se incorpora aquí sin cambios de
lógica, solo actualizando la referencia a la card v3.
...
## 4. Cómo se relacionan los 4 agentes en producción

En una implementación real, los tres primeros no compiten — **se
orquestan**: el agente basado en modelo (1) es la capa de conversación que
sostiene la memoria a lo largo de todo el intercambio; dentro de ese
flujo, delega en el razonamiento del agente basado en objetivos (2) para
saber cómo avanzar hacia una reserva confirmada; y cuando ese razonamiento
encuentra más de una alternativa viable, delega en el agente basado en
utilidad (3) para decidir cuál ofrecer primero. El agente unificado (4) es
justamente esa orquestación ya construida en un solo grafo, lista para
ejecutarse como producto. Se mantienen los 4 como scripts independientes
(mismo criterio pedagógico que `/Examples`) para que cada patrón se pueda
leer, ejecutar y evaluar por separado, y para que el `04` se pueda usar
directamente como punto de partida de un despliegue real.
...
python 04_unified_agent.py              # agente de producto: memoria + meta + recomendación rankeada
```
